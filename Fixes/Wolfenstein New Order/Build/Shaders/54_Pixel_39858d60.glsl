...
texcoord /= texcoord.w;	
	...
	vec4 shadowTexCoord;
	shadowTexCoord.x = dot4( window, _fa_[8 ] );
	shadowTexCoord.y = dot4( window, _fa_[9 ] );
	shadowTexCoord.z = dot4( window, _fa_[10 ] );
	shadowTexCoord.w = dot4( window, _fa_[11 ] );
	float shadow;
	{
		vec3 texCoord = shadowTexCoord.xyz / shadowTexCoord.w;
		{
			float s = _fa_[12 ].y * 2.0;
			vec4 shadow4;
			{
