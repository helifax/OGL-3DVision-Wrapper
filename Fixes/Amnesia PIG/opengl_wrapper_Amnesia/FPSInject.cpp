...
{
	// Get our PID
	std::string process = "aamfp.exe";
	//DWORD offset = 0x148276;
	DWORD offset = 0x1491A6; // Steam value
	std::string shellcode ("\x50\xA1\xC8\xDA\x57\x01\x83\xF8\x02\x0F\x85\x06\x00\x00\x00\x0F\x84\x10\x00\x00\x00\x83\x05\xC8\xDA\x57\x01\x01\xE9\x5E\x82\xB6\xFA\x58\x6A\x0A\x90\xC7\x05\xC8\xDA\x57\x01\x01\x00\x00\x00\xE9\x7E\x82\xB6\xFA\x58\x6A\x0A\x90\xE9\x3E\x82\xB6\xFA", 61);
	...
	// jump 1st to aamfp.exe + 14827f
	// jump 2nd to aamfp.exe + 1482b2
	DWORD64 logicJumpTo1 = baseAddress + 0x1491af; // Steam version
	DWORD64 logicJumpTo2 = baseAddress + 0x1491e2; // Steam version
	...
	// 3rd location
	writeToggleAddress = (DWORD)codeCaveAddress + 0x27;
	// Write the new memory address inside the code
	WriteProcessMemory(p, (LPVOID)writeToggleAddress, hexToggleAddress.c_str(), hexToggleAddress.length(), NULL);
	/// -------------------------------------------------------------------
