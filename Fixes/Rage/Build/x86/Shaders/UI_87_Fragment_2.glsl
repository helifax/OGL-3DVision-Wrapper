...
void main() {
	...
	light *= shadow.x;
	float graze = dot3( toViewer, globalNormal );
	graze = saturate( ( _fa_[6 ].x - graze ) / ( _fa_[6 ].x + 0.0001 ) );
	vec3 specular = vec3( 0.0);
	vec4 reflection;
	reflection.xyz = ( 2.0 * globalNormal * dot3( globalNormal, toViewer ) ) - toViewer;
	reflection.w = floor( ( 1.0 - bumpPage.x ) * 4.0 + _fa_[7 ].x );
	vec3 envColor = vec3( texCUBElod( samp_dynamicenvmap, reflection ));
	specular = envColor * _fa_[8 ].xyz;
	vec4 diffuse;
	...
	diffuse.w = 1.0;
	vec3 grazingDiffuse = mix( diffuse.xyz, _fa_[9 ].xyz, graze );
	vec3 grazingSpecular = mix( specularPage.xyz, _fa_[10 ].xyz, graze );
	vec3 color = light * ( specular * grazingSpecular + grazingDiffuse * _fa_[11 ].xyz * 2.0 );
	float fog = saturate( exp2( -clamp( length( vofi_TexCoord2.xyz - _fa_[3 ].xyz ) - _fa_[12 ].x, 0.0, _fa_[13 ].x ) * _fa_[14 ].x ) );
