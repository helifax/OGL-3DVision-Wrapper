...
void main() {
	float scale = 1.0 - _fa_[0 ].x;
	float bias = _fa_[0 ].x;
	vec4 final = tex2D( samp_transmap, vofi_TexCoord0.xy ) * vofi_Color;
	float fog = saturate( exp2( vofi_TexCoord0.z ) );
	final.xyz = mix( _fa_[1 ].xyz, final.xyz, fog );
	final.xyz = saturate( final.xyz * scale + bias );
	out_FragColor = final;
}