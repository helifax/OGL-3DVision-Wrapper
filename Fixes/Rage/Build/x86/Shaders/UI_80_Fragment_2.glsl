...
void main() {
	vec2 texcoord = screenPosToTexcoord( gl_FragCoord.xy, _fa_[0 ] );
	vec4 glareTex = h4tex2D( samp_glaremap, texcoord.xy );
	glareTex *= _fa_[1 ].x;
	...
	glareTex.w = min( glareTex.w, _fa_[2 ].x );
	out_FragColor = glareTex * glareTex.w;
	out_FragColor.w = 1.0;
}