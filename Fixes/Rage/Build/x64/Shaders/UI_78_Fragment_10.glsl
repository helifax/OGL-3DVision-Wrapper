...
void main() {
	vec2 upper = vofi_TexCoord0.xy;
	vec2 delta;
	delta.x = dFdx( upper ).x * 0.25;
	delta.y = dFdy( upper ).y * 0.25;
	vec4 color = 0.25 * (
	tex2Dbias( samp_transmap, vec4( upper.x-delta.x, upper.y-delta.y, 0, -1 ) ) +
	tex2Dbias( samp_transmap, vec4( upper.x-delta.x, upper.y+delta.y, 0, -1 ) ) +
	tex2Dbias( samp_transmap, vec4( upper.x+delta.x, upper.y+delta.y, 0, -1 ) ) +
	tex2Dbias( samp_transmap, vec4( upper.x+delta.x, upper.y-delta.y, 0, -1 ) ) );
	out_FragColor = vofi_Color;
	out_FragColor.w = color.w * vofi_Color.w;
}