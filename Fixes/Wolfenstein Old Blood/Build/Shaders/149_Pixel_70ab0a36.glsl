...
worldPos.w = normalize(worldPos.w);
...
worldPos /= worldPos.w;
	...
	float invFogAmount = saturate( ( 1.0 - fogAmount ) * ( 1.0 - fogAmount ) );
	final.w *= vofi_Color.w;
	final.w *= edgefade;
	final.w *= invFogAmount;
	final.xyz = mix( vec3( 1.0 ), final.xyz, invFogAmount );
