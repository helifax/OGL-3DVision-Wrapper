...
// Attempt 1
//wclip.x += g_eye * g_eye_separation * (viewZ - g_convergence)* wclip.w/50;
...
// Attempt 2	
dmat4 inverseMVP = dmat4(_fa_[2], _fa_[3], _fa_[4], _fa_[5]);
dmat4 MVP = inverse(inverseMVP);
worldPos = vec4(wclip * inverseMVP);
worldPos /= worldPos.w;
	
dvec4 WP_temp = worldPos * MVP;
WP_temp.x -= g_eye * g_eye_separation * (WP_temp.z - g_convergence)*0.525;
...
worldPos /= worldPos.w;
	...
	final.w *= edgefade;
	final.xyz = mix( vec3( 1.0 ), final.xyz, saturate( final.w * 2.0 ) );
	float invFogAmount = 1.0 - fogAmount;
	final.xyz = mix( vec3( 1.0 ), final.xyz, saturate( invFogAmount * invFogAmount ) );
	out_FragColor0.xyz = vec3( 1.0 ) - saturate( final.xyz );
