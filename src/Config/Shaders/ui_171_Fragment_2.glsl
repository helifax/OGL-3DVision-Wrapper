...
void main() {
	vec4 CoYCgA = tex2D( samp_transmap, vofi_TexCoord0.xy );
	...
	rgba.w = CoYCgA.y;
	rgba *= vofi_Color;
	rgba.xyz += vofi_TexCoord1.xyz * rgba.w * vofi_Color.w;
	out_FragColor0.xyz = pow3( saturate( rgba.xyz ), _fa_[0 ].x ) * vofi_Color.w;
	out_FragColor0.w = rgba.w;
}